...
}, "不允许特殊字符，只接受字母、数字和下划线.");

$.validator.addMethod("userPasswordfz", function(value, element) {
    return this.optional(element) || /^(?!\D+$)(?![^a-z]+$)(?![^A-Z]+$)\S{5,16}$/.test(value);
}, "密码需要包含字母数字及字符且需要5到16位之间.");

$.validator.addMethod("userPassword", function(value, element) {
    return this.optional(element) || /^(?!\D+$)(?![^a-z]+$)\S{5,16}$/.test(value);
}, "密码需要包含字母数字及字符且需要5到16位之间.");

$.validator.addMethod("phoneOrMobile", function(value, element) {
    return this.optional(element) || /^(13|15|17|18)\d{9}$/.test(value) || /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/.test(value);
}, "联系电话不正确");
...
}, "请填入15或18位身份证号,如37112219891124xxxx");
